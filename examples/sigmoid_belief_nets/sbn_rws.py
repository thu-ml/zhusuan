...
if __name__ == "__main__":
    ...
    latent = {'h3': [qh3_samples, log_qh3],
              'h2': [qh2_samples, log_qh2],
              'h1': [qh1_samples, log_qh1]}

    # adapting the proposal
    klpq_obj = zs.variational.klpq(log_joint, observed={'x': x_obs},
                                   latent=latent, axis=0)
    ...
    log_likelihood = tf.reduce_mean(
        zs.is_loglikelihood(log_joint, {'x': x_obs}, latent, axis=0))

    learning_rate_ph = tf.placeholder(tf.float32, shape=[], name='lr')
    optimizer = tf.train.AdamOptimizer(learning_rate_ph, epsilon=1e-4)
    klpq_grads = optimizer.compute_gradients(klpq_cost)
