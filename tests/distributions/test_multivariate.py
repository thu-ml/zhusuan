...
class TestMultinomial(tf.test.TestCase):
    ...
    def test_init_n(self):
        ...
        with self.test_session(use_gpu=True) as sess:
            ...
            n_experiments = tf.placeholder(tf.int32, 10)
            dist2 = Multinomial(logits, 2)
            self.assertEqual(
                sess.run([dist.n_categories, dist.n_experiments],
                         feed_dict={logits: np.ones([2]),
                                    n_experiments: 10}),
                [2, 10])
    ...
    def test_value_shape(self):
        ...
        with self.test_session(use_gpu=True):
            self.assertEqual(dist2._value_shape().eval(
                feed_dict={logits: np.ones([2])}).tolist(), [10])
