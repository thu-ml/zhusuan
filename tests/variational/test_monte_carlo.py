...
class TestImportanceWeightedObjective(tf.test.TestCase):
    ...
    def test_objective(self):
        ...
        def _check_k1_elbo(x_mean, x_std):
            ...
            lower_bound = \
                importance_weighted_objective(log_joint,
                                              observed={},
                                              latent={'x': [qx_samples_n1,
                                                            log_qx_n1]},
                                              axis=0)
        ...
        def _check_monotomous_elbo(x_mean, x_std):
            # check their elbo
            def log_joint(observed):
                norm = Normal(mean=x_mean, std=x_std)
                return norm.log_prob(observed['x'])
            ...
            with self.test_session(use_gpu=True) as sess:
                a = sess.run(lower_bound)
                b = sess.run(analytic_lower_bound)
                self.assertTrue(a > b - 1e-2)

        _check_monotomous_elbo(0., 1.)
        _check_monotomous_elbo(2., 3.)
    ...
    def test_vimco(self):
        ...
        v_qx_samples = eps_samples * tf.stop_gradient(sigma) + \
                       tf.stop_gradient(mu)
        v_norm = Normal(mean=mu, std=sigma)
        v_log_qx = v_norm.log_prob(v_qx_samples)
        ...
        def _check_vimco(x_mean, x_std, threshold):
            ...
            v_lower_bound = \
                importance_weighted_objective(log_joint,
                                              observed={},
                                              latent={'x': [v_qx_samples,
                                                            v_log_qx]},
                                              axis=0)

            sgvb_cost = v_lower_bound.vimco()
            sgvb_cost = tf.reduce_mean(sgvb_cost)
            sgvb_grads = tf.gradients(sgvb_cost, [mu, sigma])
            true_cost = tf.reduce_mean(lower_bound.sgvb())
            true_grads = tf.gradients(true_cost, [mu, sigma])
        ...
        _check_vimco(0., 1., 0.04)
        _check_vimco(2., 3., 0.02)
