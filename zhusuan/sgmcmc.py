...
class SGMCMC(object):
    ...
    The implementation framework is similar to that of :class:`~zhusuan.hmc.HMC`
    class. However, SGMCMC algorithms do not include Metropolis update, and
    typically do not include hyperparameter adaptation, so the implementation
    here is easier to read than that of :class:`~zhusuan.hmc.HMC` class.
    ...
    The typical code for SGMCMC inference is like::
        ...
        with tf.Session() as sess:
            for _ in range(n_iters):
                _, w, info = sess.run([sample_op, new_w, sample_info], feed_dict=...)

    .. note::

        The difference with :class:`~zhusuan.hmc.HMC` in usage is that one needs
        to call :meth:`make_grad_func` first, then call :meth:`sample` with a
        `grad_func`. This design is intended to enable the user to customize the
        gradient function, since the user may apply some variance reduction
        technique to the stochastic gradient when using SGMCMC.
    ...
    def make_grad_func(self, meta_bn, observed, latent):
        ...
        :param meta_bn: A function or a
            ...
            More conveniently, the user can also provide a
            :class:`~zhusuan.framework.meta_bn.MetaBayesianNet` instance instead
            of directly providing a log_joint function. Then a log_joint
            function will be created so that `log_joint(obs) =
        ...
        :return: The same gradient function whose arguments are the `Variable`
            list above **and** a `observed` dictionary, to allow users to create a
            gradient function with dataset splitting in case of large batch size
            and limited memory.
    ...
    def sample(self, grad_func=None):
        ...
        :param grad_func: A function that accepts a list argument of tensorflow
            ...
            argument is not provided, then the default gradient function created
            in :meth:`make_grad_func` is used.
        ...
        :return: A dictionary that records some useful statistics (only for SGHMC
            and SGNHT).
...
class SGLD(SGMCMC):
    ...
    (Welling & Teh, 2011) (SGLD) update. The updating equation implemented below
    follows Equation (3) in the paper.

    :param learning_rate: A 0-D `float32` Tensor. It can be either a constant or
        a placeholder for decaying learning rate.
...
class SGHMC(SGMCMC):
    ...
    :param variance_estimate: A 0-D `float32` Tensor corresponding to
        ...
        the gradient variance well. Note that `variance_estimate` must be smaller
        than `friction`.
    :param n_iter_resample_v: A 0-D `int32` Tensor. Each `n_iter_resample_v`
        calls to the sampling operation, the momentum variable will be resampled
        from the corresponding normal distribution once. Smaller
...
class SGNHT(SGMCMC):
    ...
    equation implemented below follows Algorithm 2 in the supplementary material
    of the paper. A 2nd-order integrator introduced in (Chen et al., 2015) is
    supported.
    ...
    :param variance_extra: A 0-D `float32` Tensor corresponding to :math:`a` in
        ...
        initial friction :math:`\\alpha` will be set to this value). Normally just
        set it to zero.
    ...
    def __init__(self, learning_rate, variance_extra=0., tune_rate=1.,
        ...
        self.lr = tf.convert_to_tensor(learning_rate, tf.float32,
                                        name="learning_rate")
        self.a = tf.convert_to_tensor(variance_extra, tf.float32,
                                          name="variance_extra")
    ...
    def _define_variables(self, qs):
        ...
        if self.use_vector_alpha:
            self.alphas = [tf.Variable(self.a*tf.ones(tf.shape(q))) for q in qs]
        else:
    ...
    def _update(self, qs, grad_func):
        ...
        old_vs = [tf.cond(tf.equal(self.n_iter_resample_v, 0), lambda: v, lambda: tf.cond(
            ...
            lambda: resample_momentum(v), lambda: v)) for v in self.vs]
        gaussian_terms = [tf.random_normal(
            tf.shape(old_v), stddev=tf.sqrt(2*self.a*self.lr)) for old_v in old_vs]
        if not self.second_order:
            new_vs = [(1 - alpha) * old_v + self.lr * grad + gaussian_term
                for (old_v, alpha, grad, gaussian_term) in zip(old_vs, self.alphas, grad_func(qs), gaussian_terms)]
            new_qs = [q + new_v for (q, new_v) in zip(qs, new_vs)]
            mean_ks = [maybe_reduce_mean(new_v**2) for new_v in new_vs]
            new_alphas = [alpha + self.tune_rate * (mean_k - self.lr) for (alpha, mean_k) in zip(self.alphas, mean_ks)]
        else:
            q1s = [q + 0.5 * old_v for (q, old_v) in zip(qs, old_vs)]
            mean_k1s = [maybe_reduce_mean(old_v**2) for old_v in old_vs]
            alpha1s = [alpha + 0.5 * self.tune_rate * (mean_k1 - self.lr) for (alpha, mean_k1) in zip(self.alphas, mean_k1s)]
            ...
            new_qs = [q1 + 0.5 * new_v for (q1, new_v) in zip(q1s, new_vs)]
            mean_ks = [maybe_reduce_mean(new_v**2) for new_v in new_vs]
            new_alphas = [alpha1 + 0.5 * self.tune_rate * (mean_k - self.lr) for (alpha1, mean_k) in zip(alpha1s, mean_ks)]

        infos = [{"mean_k": tf.reduce_mean(mean_k), "alpha": tf.reduce_mean(new_alpha)} for (mean_k, new_alpha) in zip(mean_ks, new_alphas)]

        with tf.control_dependencies(new_vs + new_qs + new_alphas):
            update_qs = [q.assign(new_q) for (q, new_q) in zip(qs, new_qs)]
            update_vs = [v.assign(new_v) for (v, new_v) in zip(self.vs, new_vs)]
            update_alphas = [alpha.assign(new_alpha) for (alpha, new_alpha) in zip(self.alphas, new_alphas)]

        update_ops = [tf.group(update_q, update_v, update_alpha)
            for (update_q, update_v, update_alpha) in zip(update_qs, update_vs, update_alphas)]
