...
class StochasticTensor(TensorArithmeticMixin):
    ...
    @overload_name_param('StochasticTensor')
    def __init__(self, distribution, n_samples, observed=None, name=None):
        self._name = name
    ...
    def tensor(self):
        ...
        if not hasattr(self, '_tensor'):
            ...
            elif self._net and self._name in self._net.observed:
                try:
...
class BayesianNet(Context):
    ...
    * Stochastic nodes, constructed by :class:`StochasticTensor`.
    ...
    def _add_stochastic_tensor(self, s_tensor):
        ...
        """
        if not s_tensor.name:
            raise ValueError("A name of the StochasticTensor must be provided")
        elif s_tensor.name in self._stochastic_tensors:
