...
def advi_old(model, observed_inputs, observed_layers, latent_layers,
             given_layers, optimizer=tf.train.AdamOptimizer()):
    """
    Implements the automatic differentiation variational inference (ADVI)
    algorithm. For now we assume all latent variables have been transformed in
    the model definition to have their support on R^n.
    ...
    discrete_latent_layers = dict(
        filter(lambda x: isinstance(x[1], Discrete),
    ...
    if discrete_latent_layers:
        ...
        # cartesian products
        all_disc_latent_configs = itertools.product(
            *all_disc_latent_configs)
        ...
        for discrete_latent_inputs in all_disc_latent_configs:
            ...
            latent_outputs = dict(
                zip(latent_k, map(lambda x: x[0], outputs)))
            # size: continuous layer (batch_size, n_samples)
            #       discrete layers (batch_size, 1)
            latent_logpdfs = dict(
                zip(latent_k, map(lambda x: x[1], outputs)))
        ...
        latent_outputs = dict(
            zip(latent_k, map(lambda x: x[0], outputs[:len(latent_v)])))
        given_outputs = dict(
            zip(given_k, map(lambda x: x[0], outputs[len(latent_v):])))
